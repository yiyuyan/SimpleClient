...
import org.ksmcbrigade.hacks.XRay;
...
public class Block
{
    ...
    {
        int ret = this.lightOpacity;
        Hack hack = Category.RENDER.get("BlockLight");
        if(hack!=null && hack.enabled){
            if(Arrays.asList(XRay.blocks).contains(state.getBlock())){
                ret = 0;
            }
        }
        return ret;
